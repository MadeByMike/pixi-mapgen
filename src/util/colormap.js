...
function smoothColoring(e, t, m) {
  ...
  if (e < 0.0) {
    return `rgb(${(48 + 48 * e) | 0}, ${(64 + 64 * e) | 0}, ${(127 + 128 * e) |
      0})`;
  }
...
class Coloring {
  draw_coast_s(map, s) {
    return (
      map.r_ocean[map.mesh.s_begin_r(s)] !== map.r_ocean[map.mesh.s_end_r(s)]
    );
  }
  ...
  draw_river_s(map, s) {
    let r0 = map.mesh.s_begin_r(s),
      r1 = map.mesh.s_end_r(s);
    return (
      (map.s_flow[s] > 0 || map.s_flow[map.mesh.s_opposite_s(s)] > 0) &&
      !map.r_water[r0] &&
      !map.r_water[r1]
    );
  }
